...
$Output = $cluster | ForEach-Object {
    ...
    [pscustomobject]@{
        ...
        AvailableSlots    = $_.HAAvailableSlots
        AdmissionControl  = $_.HAAdmissionControlEnabled
        HostMonitoring    = $_.ExtensionData.Configuration.DasConfig.HostMonitoring
        HeartbeatDatastore= $_.ExtensionData.Configuration.DasConfig.HBDatastoreCandidatePolicy

    }

} | Where-Object {!$_.HAenabled -or $_.AvailableSlots -lt $MinimumSlotsAvailable -or !$_.AdmissionControl -or $_.HeartbeatDatastore -eq 0 -or $_.HostMonitoring -ne "enabled"}
